...
def resize_according_to_long_side(img, box, seg, size):
...
def resize_according_to_short_side(img, box, seg, size):
    h, w = img.height, img.width
...
class RandomHorizontalFlip(object):
    def __call__(self, input_dict):
        if random.random() < 0.5:
            img = input_dict['img']
            box = input_dict['box']
            seg = input_dict['seg'].view(-1, 2)
            text = input_dict['text']
            r1_word = input_dict['subj']

            img = F.hflip(img)
            text = text.replace('right', '*&^special^&*').replace('left', 'right').replace('*&^special^&*', 'left')
            r1_word = r1_word.replace('right', '*&^special^&*').replace('left', 'right').replace('*&^special^&*', 'left')
            h, w = img.height, img.width
            box = box[[2, 1, 0, 3]] * torch.as_tensor([-1, 1, -1, 1]) + torch.as_tensor([w, 0, w, 0])
            seg = seg * torch.as_tensor([-1, 1]) + torch.as_tensor([w, 0])

            input_dict['img'] = img
            input_dict['box'] = box
            input_dict['seg'] = seg.view(-1)
            input_dict['text'] = text
            input_dict['subj'] = r1_word
...
class RandomResize(object):
    ...
    def __call__(self, input_dict):
        ...
        if self.with_long_side:
            resized_img, resized_box, resized_seg = resize_according_to_long_side(img, box, seg, size)
        else:
            resized_img, resized_box, resized_seg = resize_according_to_short_side(img, box, seg, size)

        input_dict['img'] = resized_img
        input_dict['box'] = resized_box
        input_dict['seg'] = resized_seg
        return input_dict
...
class NormalizeAndPad(object):
    ...
    def __call__(self, input_dict):
        ...
        if 'seg' in input_dict.keys():
            seg = input_dict['seg'].view(-1, 2)
            seg = seg + torch.as_tensor([left, top])
            input_dict['seg'] = seg.view(-1)
